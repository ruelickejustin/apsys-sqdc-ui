<script>
  ...
  import { meta, pickWs } from "../lib/state";

  // später aus Excel/JSON: hier Demo
  const list = ["WS01","WS02","WS03","WS04"].map(id=>({id, name:`Workstation ${id}`}));

  function open(ws){ pickWs($meta.epu, ws.id); goto("/board/"+ws.id); }
</script>
